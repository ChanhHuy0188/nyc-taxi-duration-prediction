...
from .kafka_source import build_source
from .base import FlinkJob
from .metrics import RequestCounter, logger
...
class SchemaValidationJob(FlinkJob):
    def __init__(self):
        self.jars_path = f"{os.getcwd()}/flink/jars/"
        self.input_topics = os.getenv(
            "KAFKA_INPUT_TOPICS", "tracking.raw_data"
        ).split(",")
        self.group_id = os.getenv("KAFKA_GROUP_ID", "flink-group")
        self.valid_topic = os.getenv(
            "KAFKA_VALID_TOPIC", "tracking.data.validated"
        )
        self.invalid_topic = os.getenv(
            "KAFKA_INVALID_TOPIC", "tracking.data.invalid"
        )
